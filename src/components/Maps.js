...
mapboxgl.accessToken =
  "pk.eyJ1IjoibWF0ZWptZWdsaWMiLCJhIjoiY2tiZXpudm1iMHFvZDJ1cG1ub3pqaHZhMyJ9.lzD5DdQCear_9OR586acJQ"
...
var mapboxConfig = [
  // settings for map based on the device (screen width)
...
var baseLayers = [
  // mapbox config for changing backgrounds
  {
  ...
  componentDidMount() {
    ...
    function DefineSiteLanguage() {
      if (sitePath.length === 1) {
        // special handling for homepage, onChange modify layout.js as well
        lang = "hr"
        originalPath = "/"
      }
      if (sitePath.substr(1, 3) === "slo") {
        // repeat for posts pages
        lang = "slo"
        charPosition = 5
        originalPath = "/slo/"
      }
    ...
    function PeakPopUp() {
      ...
      el.id = "marker"
      new mapboxgl.Marker(el).setLngLat(peakCoordinates).addTo(map)
    ...
    function RemoveIcons() {
      ...
      iconsShown.forEach(function (marker) {
        // duplicated same shit not completely cleaning array
        marker.remove()
        removeFromArray(iconsShown, marker)
      })
    }
    ...
    function ShowSelectedRoute(e) {
      ...
      if (oneRouteCollection[0] === undefined) {
        // safety: do not allow to focus (open route) on hidden paths
        return
      }
      // remove alternative routes from collection (different styling)
      if (oneRouteCollection[0].properties.alternative !== null) {
        oneRouteCollection[0].properties.alternative.forEach(function (
          alternativeRoute
        ) {
      ...
      map.setPaintProperty(
        "lines-layer",
        "line-width",
        mapboxConfigObject.limitedLineWidth
      )
      mapSetPaintProperty()
      ...
      map.setPaintProperty(
        "lines-layer-one",
        "line-width",
        mapboxConfigObject.selectedLineWidth
      )
      clickOnRoute()
      showPopUp(currentVisibleMapLayers)
      // update index.js
    ...
    function setAlternatives(
      ...
      if (oneRouteCollection[0].properties[prop] !== null) {
        oneRouteCollection[0].properties[prop].forEach(function (
          alternativeRoute
        ) {
    ...
    map.on("load", function () {
      ...
      buttonDeleteRoutes.addEventListener("click", function () {
        ...
        currentVisibleMapLayers.forEach(function (el) {
          //twice due to glith with forEach - gotta investigate
          resetSource(el, el)
        })
        currentVisibleMapLayers.forEach(function (el) {
          resetSource(el, el)
